...
| **Free-response channels** | You can make specific channels mention-free with `DISCORD_FREE_RESPONSE_CHANNELS`, or disable mentions globally with `DISCORD_REQUIRE_MENTION=false`. Free-response controls mention gating only; `DISCORD_AUTO_THREAD=true` can still create threads unless the channel is also listed in `DISCORD_NO_THREAD_CHANNELS`. |
...
| `DISCORD_AUTO_THREAD` | No | `true` | When `true`, automatically creates a new thread for handled messages in a text channel, so each conversation is isolated (similar to Slack behavior). Messages already inside threads or DMs are unaffected. |
...
Free-response controls whether an `@mention` is required; it does not by itself disable threading. If `discord.auto_thread` is enabled, mention-free messages in these channels can still start a thread. Add the channel to `discord.no_thread_channels` only when you explicitly want inline replies.
