...
def adapter(monkeypatch):
    ...
    for _var in (
        ...
        "DISCORD_AUTO_THREAD",
        "DISCORD_AUTO_THREAD_ARCHIVE_DURATION",
...
def test_discord_auto_thread_archive_duration_uses_env(adapter, monkeypatch):
    monkeypatch.setenv("DISCORD_AUTO_THREAD_ARCHIVE_DURATION", "60")

    assert adapter._auto_thread_archive_duration() == 60


def test_discord_auto_thread_archive_duration_rejects_invalid_values(adapter, monkeypatch):
    monkeypatch.setenv("DISCORD_AUTO_THREAD_ARCHIVE_DURATION", "17")
...
async def test_discord_free_response_channel_still_auto_threads(adapter, monkeypatch):
    """Free-response bypasses mention gating only; no_thread_channels controls inline replies."""
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "true")
    ...
    message = make_message(
        channel=FakeTextChannel(channel_id=789),
        content="wiki source drop in free-response channel",
    )
    ...
    adapter.handle_message.assert_awaited_once()
    event = adapter.handle_message.await_args.args[0]
    assert event.text == "wiki source drop in free-response channel"
    assert event.source.chat_type == "thread"
    assert event.source.thread_id == "999"


@pytest.mark.asyncio
async def test_discord_no_thread_channel_skips_auto_thread_even_when_free_response(adapter, monkeypatch):
    """no_thread_channels remains the explicit inline-reply escape hatch."""
    monkeypatch.setenv("DISCORD_REQUIRE_MENTION", "true")
    monkeypatch.setenv("DISCORD_FREE_RESPONSE_CHANNELS", "789")
    ...
    adapter.handle_message.assert_awaited_once()
    event = adapter.handle_message.await_args.args[0]
    assert event.text == "admin inline chat"
    assert event.source.chat_type == "group"
